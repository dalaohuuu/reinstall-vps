...
build_extra_cmdline() {
    ...
    # 传递 hostname / timezone / cache-size 给安装器
    if [ -n "$hostname_new" ]; then
        value="$hostname_new"
        if is_need_quote "$value"; then
            extra_cmdline+=" extra_hostname='$value'"
        else
            extra_cmdline+=" extra_hostname=$value"
        fi
    fi
    ...
    if [ -n "$timezone_new" ]; then
        value="$timezone_new"
        if is_need_quote "$value"; then
            extra_cmdline+=" extra_timezone='$value'"
        else
            extra_cmdline+=" extra_timezone=$value"
        fi
    fi
    ...
    if [ -n "$cache_size_mb" ]; then
        ...
        # 数字一般不需要引号，但这里沿用统一处理
        if is_need_quote "$value"; then
            extra_cmdline+=" extra_cache_size='$value'"
        else
            extra_cmdline+=" extra_cache_size=$value"
        fi
    fi
...
# 规范化 timezone 参数
if [ -n "$timezone_new" ]; then
    case "$timezone_new" in
    shanghai|Shanghai|SHANGHAI|asia/shanghai|Asia/Shanghai)
        timezone_new="Asia/Shanghai"
        ;;
    esac
fi
...
# cache-size 必须是数字
if [ -n "$cache_size_mb" ]; then
    if ! is_digit "$cache_size_mb"; then
        error_and_exit "--cache-size 必须是数字（单位 MB），当前值: $cache_size_mb"
    fi
fi
